 ...
 * @author Jeff
 */
public class WeakenCurseSkillAttack extends CircleAttack{
    ...
    public WeakenCurseSkillAttack(CircleEntity owner, int lf, int dmg, double x, double y,
                ...
                if(intersectsCircle(player))
                {
                    if(!hitAlready.contains(player))
                    {
                        ...
                        for (Iterator<CircleEffect> iterator = player.effects().iterator(); iterator.hasNext();) {
                            ...
                            if(effect instanceof WeakEffect){
                        ...
                        if(addEffect)
                            player.effects().add(new WeakEffect(curseLife));
            ...
            for(CircleEntity monster : world.monsters())
            {
                if(intersectsCircle(monster))
                {
                    if(!hitAlready.contains(monster))
                    {
                        ...
                        if(addEffect)
                            monster.effects().add(new WeakEffect(curseLife));
