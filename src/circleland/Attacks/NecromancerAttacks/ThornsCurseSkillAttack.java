...
import circleland.Effects.ThornsEffect;
import circleland.Effects.WeakEffect;
...
public class ThornsCurseSkillAttack extends CircleAttack{
    ...
    public ThornsCurseSkillAttack(CircleEntity owner, int lf, int dmg, double x, double y,
                ...
                if(intersectsCircle(player))
                {
                    if(!hitAlready.contains(player))
                    {
                        ...
                        for (Iterator<CircleEffect> iterator = player.effects().iterator(); iterator.hasNext();) {
                            ...
                            if(effect instanceof ThornsEffect){
                        ...
                        if(addEffect)
                            player.effects().add(new ThornsEffect(curseLife));
            ...
            for(CircleEntity monster : world.monsters())
            {
                if(intersectsCircle(monster))
                {
                    if(!hitAlready.contains(monster))
                    {
                        ...
                        if(addEffect)
                            monster.effects().add(new ThornsEffect(curseLife));
