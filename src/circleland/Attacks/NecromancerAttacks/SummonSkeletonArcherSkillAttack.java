...
import circleland.Classes.Summons.SummonedSkeletonArcher;
 ...
 * @author Jeff
 */
public class SummonSkeletonArcherSkillAttack extends CircleAttack {
    private static final Color OUTLINE_COLOR = Color.WHITE;
    private int level;
    private ArrayList<CircleSummoned> summons;
    public SummonSkeletonArcherSkillAttack(CircleEntity owner, int life, int lvl, double x, double y,
        ...
        //check for collision with player or monster
            for (Iterator<CircleEntity> iterator = world.corpses().iterator(); iterator.hasNext();) {
                ...
                if(intersectsCircle(corpse) && !(corpse instanceof CircleSummoned))
                {
                    ...
                    //add items to world
                    //destroy attack
                    CircleSummoned cE = new SummonedSkeletonArcher(level);
